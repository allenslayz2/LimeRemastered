...
if (empty($url)) {
    if (empty($startURL)) {
      die("<style> body{margin: 0px auto;text-align: center;} body {background: url('/wallpaper.jpg');} body {color: #ff69b4;} h1 {color: #ffffff;}></style><html><head><title>AllenSlayz Proxy</title></head><body><h1>Titanium Network Proxy V2</h1>	You can visit directly visit websites by doing the following: <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "</a><br /><br />Put a website in here:<br /><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Visit Website!\" /></form><a>Most websites are unblocked except for some that wont work no matter what like YouTube!</body></html>");
    } else {
      $url = $startURL;
    }
